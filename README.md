...
| ``auth_pass`` | no       |            | Specifies the password to use if required to enter privileged mode on the remote device; if *authorize* is set to no, this variable does not apply; if value is unspecified, the ANSIBLE_NET_AUTH_PASS environment variable value is used  |
